...
contract ComptrollerStorage {
    struct LiquidationOrder {
        LtToken ltTokenCollateral;
        LtToken ltTokenBorrowed;
        uint256 repayAmount;
    }
     ...
     */
    mapping(address => LtToken[]) public accountAssets;

    /**
     * @notice Official mapping of ltTokens -> Market metadata
     * @dev Used e.g. to determine if a market is supported
     */
    mapping(address => Market) public markets;

    /// @notice A list of all markets
    LtToken[] public allMarkets;

    /// @notice Borrow caps enforced by borrowAllowed for each ltToken address. Defaults to zero which restricts borrowing.
    mapping(address => uint256) public borrowCaps;

    /// @notice Minimal collateral required for regular (non-batch) liquidations
    uint256 public minLiquidatableCollateral;

    /// @notice Supply caps enforced by mintAllowed for each ltToken address. Defaults to zero which corresponds to minting not allowed
