 ...
 * any existing pool like `getLtTokenForAsset` and `getPoolsSupportedByAsset`. It also contains methods for updating pool metadata (`updatePoolMetadata`)
...
contract PoolRegistry is Ownable2StepUpgradeable, AccessControlledV8, PoolRegistryInterface {
    ...
    /**
     * @dev Maps pool's comptroller address to asset to ltToken.
     */
    mapping(address => mapping(address => address)) private _ltTokens;
     ...
     */
    event MarketAdded(address indexed comptroller, address indexed ltTokenAddress);
     ...
     * @custom:error ZeroAddressNotAllowed is thrown when ltToken address is zero
     * @custom:error ZeroAddressNotAllowed is thrown when ltTokenReceiver address is zero
     */
    function addMarket(AddMarketInput memory input) external {
        _checkAccessAllowed("addMarket(AddMarketInput)");
        ensureNonzeroAddress(address(input.ltToken));
        ensureNonzeroAddress(input.ltTokenReceiver);
        require(input.initialSupply > 0, "PoolRegistry: initialSupply is zero");

        LtToken ltToken = input.ltToken;
        address ltTokenAddress = address(ltToken);
        address comptrollerAddress = address(ltToken.comptroller());
        Comptroller comptroller = Comptroller(comptrollerAddress);
        address underlyingAddress = ltToken.underlying();
        ...
        require(
            _ltTokens[comptrollerAddress][underlyingAddress] == address(0),
            "PoolRegistry: Market already added for asset comptroller combination"
        );

        comptroller.supportMarket(ltToken);
        comptroller.setCollateralFactor(ltToken, input.collateralFactor, input.liquidationThreshold);

        uint256[] memory newSupplyCaps = new uint256[](1);
        uint256[] memory newBorrowCaps = new uint256[](1);
        LtToken[] memory ltTokens = new LtToken[](1);

        newSupplyCaps[0] = input.supplyCap;
        newBorrowCaps[0] = input.borrowCap;
        ltTokens[0] = ltToken;

        comptroller.setMarketSupplyCaps(ltTokens, newSupplyCaps);
        comptroller.setMarketBorrowCaps(ltTokens, newBorrowCaps);

        _ltTokens[comptrollerAddress][underlyingAddress] = ltTokenAddress;
        ...
        underlying.forceApprove(ltTokenAddress, 0);
        underlying.forceApprove(ltTokenAddress, amountToSupply);
        ltToken.mintBehalf(input.ltTokenReceiver, amountToSupply);

        emit MarketAdded(comptrollerAddress, ltTokenAddress);
    }
    ...
    function getLtTokenForAsset(address comptroller, address asset) external view override returns (address) {
        return _ltTokens[comptroller][asset];
    }
